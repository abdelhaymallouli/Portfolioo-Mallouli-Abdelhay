...
import { BEWERBUNG, EXPERIENCE, LANGUAGES, SITE } from "@/data/content";
  ...
  return (
    <PageShell>
      <PageHeader width="reading">
        ...
        {/* Header */}
        <Reveal delay={0.05}>
          <p className="mt-10 font-mono text-xs uppercase tracking-[0.12em] text-muted">
            {BEWERBUNG.eyebrow}
          </p>

          <h1 className="mt-6 text-balance text-[2.25rem] font-medium leading-[1.1] tracking-[-0.035em] text-ink sm:text-[3rem]">
            {BEWERBUNG.headline}
          </h1>
          ...
          <section className="mt-20 border-t border-line pt-12">
            ...
            <ul className="mt-8 space-y-5">
              {BEWERBUNG.pitch.map((point) => (
                <li
                  key={point}
                  className="flex gap-4 text-pretty text-[1.0625rem] leading-[1.7] text-secondary"
                >
                  <span
                    aria-hidden="true"
                ...
                <li key={`${job.company}-${job.period}`}>
                  ...
                  <ul className="mt-4 space-y-2.5">
                    {job.achievements.map((achievement) => (
                      <li
                        key={achievement}
                        className="flex gap-3 text-pretty text-[0.9375rem] leading-[1.65] text-secondary"
                      >
                        <span
                          aria-hidden="true"
                  ...
                  <ul className="mt-4 flex flex-wrap gap-x-5 gap-y-2">
                    {job.stack.map((tech) => {
                      ...
                      return (
                        <li
                          key={tech}
                          className="flex items-center gap-1.5 text-[0.8125rem] text-secondary"
                        >
                          <Icon
          ...
          <section className="mt-20 border-t border-line pt-12">
            ...
            <ul className="mt-8">
              {LANGUAGES.map((lang) => (
                ...
                >
                  <span className="text-[0.9375rem] text-ink">{lang.name}</span>
        ...
        <Reveal delay={0.3}>
          <section className="mt-20 border-t border-line pt-12">
            <h2 className="text-[1.5rem] font-medium tracking-[-0.025em] text-ink">
              Let&apos;s talk.
            </h2>
